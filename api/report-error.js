...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!webhookUrl) {
      console.error(
        "[ErrorReporting] DISCORD_ERROR_WEBHOOK_URL not configured",
      );
      return res
        .status(503)
    ...
    if (!payload?.error) {
      return res
        .status(400)
        .json({ error: "Invalid payload: missing error field" });
    }
    ...
    const sanitizedStack = payload.stack
      ? sanitizeDiscordText(payload.stack)
      : null;
    const sanitizedVersion = sanitizeDiscordText(
      payload.context?.version || "unknown",
    );
    const sanitizedPlatform = sanitizeDiscordText(
      payload.context?.platform || "unknown",
    );
    const sanitizedNode = sanitizeDiscordText(
      payload.context?.nodeVersion || "unknown",
    );
    ...
    if (existing && existing.expires > now) {
      ...
      console.log(
        `[ErrorReporting] Duplicate error (id=${computedId}) suppressed; count=${existing.count}`,
      );
