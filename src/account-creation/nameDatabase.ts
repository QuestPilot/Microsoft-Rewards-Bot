import { secureRandomPick } from "../util/security/SecureRandom";
...
export function getRandomFirstName(): string {
  ...
  const category = secureRandomPick(
    categories,
  ) as keyof typeof NAME_DATABASE.firstNames;
  const names = NAME_DATABASE.firstNames[category];
  return secureRandomPick(names) ?? "Alex";
}

export function getRandomLastName(): string {
  const names = NAME_DATABASE.lastNames;
  return secureRandomPick(names) ?? "Smith";
}
